...
Contributions are welcome! Bug reports, feature ideas, and pull requests can be submitted through the [issue tracker](https://github.com/ruflas/headset-battery-indicator/issues) and [pull requests](https://github.com/ruflas/headset-battery-indicator/pulls).

For code changes:

1. Fork the repository and create a branch from `main`.
2. Keep the app working on both **Linux** and **Windows**.
...
The UI is translatable with Qt's translation system. New languages are very welcome — **you only need to submit a `.ts` file**. There is no need to install Qt Linguist or compile anything: a GitHub Actions workflow compiles `.ts` files to `.qm` automatically once the pull request is merged, and the new language appears in **Preferences → Language** on its own.

Translation files live in [`headset_battery_indicator/translations/`](headset_battery_indicator/translations) and are named `headset_battery_indicator_XX.ts`, where `XX` is the language code (e.g. `de`, `fr`, `gl`, `pt_BR`).

#### Adding a new language

**Option A — copy an existing translation (no tooling needed):**

1. Copy [`headset_battery_indicator_es.ts`](headset_battery_indicator/translations/headset_battery_indicator_es.ts) to `headset_battery_indicator_XX.ts`.
2. Change the `language="es_ES"` attribute in the `<TS>` header to your locale (e.g. `language="de_DE"`).
3. Translate only the text inside each `<translation>` tag. Leave `<source>` untouched — it is always the original English string.

**Option B — generate a fresh file from source (requires PySide6):**

```bash
pyside6-lupdate headset_battery_indicator -ts headset_battery_indicator/translations/headset_battery_indicator_XX.ts
```

Then fill in the `<translation>` tags as in Option A. You can edit the file in any text editor, or with Qt Linguist (`pyside6-linguist`) if you prefer a graphical tool.

#### Tips

- Keep `{}` placeholders exactly as they appear in the source string — they are replaced with values at runtime.
- The file is XML, so keep special characters escaped (e.g. `&amp;` for `&`).
- If a `<translation>` has `type="unfinished"`, remove that attribute once the string is translated.
- Only commit the `.ts` file — `.qm` files are generated by CI.

#### Updating an existing translation

When new strings are added to the app, existing `.ts` files can be refreshed without losing current translations:

```bash
pyside6-lupdate headset_battery_indicator -ts headset_battery_indicator/translations/headset_battery_indicator_XX.ts
```

New strings will appear as `unfinished` and are ready to be translated.
